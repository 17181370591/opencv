...
px=img[100,100]                     #获取一个点的rgb
print(px)
blue=img[100,100,0]                 #获取一个点的b
print(blue)

Numpy 是经过优化了的进行快速矩阵运算的软件包。所以我们不推荐逐个获取像素值并修改，这样会很慢，
能有矩阵运算就不要用循环。
注意：上面提到的方法被用来选取矩阵的一个区域，比如说前 5 行的后 3列。对于获取每一个像素值，
也许使用 Numpy 的 array.item() 和 array.itemset() 会更好。但是返回值是标量。如果你想获得所有 B，G，R 的
...
for i in range(1,111):
    for j in range(1,111):
        img.itemset((i,j,0),220)                        #设置一个点的r（好像是）
...
img[140:225,746:890]=ball                   #直接用切片进行拷贝，经常被使用
...
plt.subplot(236),plt.imshow(constant,'gray'),plt.title('CONSTANT')          #会这个就够了
plt.show()
...
按位运算
这里包括的按位操作有：AND，OR，NOT，XOR 等。当我们提取图像的一部分，
选择非矩形 ROI 时这些操作会很有用（下一章你就会明白）。
下面的例子就是教给我们如何改变一幅图的特定区域。我想把 OpenCV 的标志放到另一幅图像上。
...
#按位运算要求两个图尺寸一样，所以从img1截取了img2的尺寸大小进行操作

img2gray = cv2.cvtColor(img2,cv2.COLOR_BGR2GRAY)
#得到img2的灰度图的副本，目的是绘制mask

ret, mask = cv2.threshold(img2gray, 76,255, cv2.THRESH_BINARY)
mask_inv = cv2.bitwise_not(mask)
#得到mask和mask的异或数组
#目的是用mask与a求位与得到x，用mask_inv与b求位与得到y，
#然后cv2.add(x,y)得到z，z就是b透过mask复制到a上的图片

img1_bg = cv2.bitwise_and(roi,roi,mask = mask_inv)          #x  
img2_fg = cv2.bitwise_and(img2,img2,mask = mask)            #y    

dst = cv2.add(img1_bg,img2_fg)                              #z
img1[0:rows, 0:cols ] = dst                     #复制回原图
cv2.imshow('res',img1)
