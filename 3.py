...
img1 = cv2.drawContours(img1,[box],0,(0,255,255),5)
w,h=ra(box[0],box[1]),ra(box[2],box[1])
#边界矩形的宽高比
aspect_ratio = float(w)/h
print('边界矩形的宽高比：',aspect_ratio)
#轮廓面积与边界矩形面积的比
area = cv2.contourArea(contours[xx])
...
cv2.circle(img1,tuple(np.int0(rect[0])),int(equi_diameter/2),(0,0,255),5)
#对象的方向，下面的方法还会返回长轴和短轴的长度
ellipse1= cv2.fitEllipse(contours[xx])
print('旋转矩形的内切椭圆的信息：',ellipse1)
